let films = [{
	...
	},
	{
		...
		release_date: 9940342800,
		genres: ['Music', 'Documentary'],
	},
];


let mainListFirst = document.querySelector("#mainId-list")

function render(arrFilm, elemenFilm) {
	elemenFilm.innerHTML = null
	for (let i = 0; i < arrFilm.length; i++) {
		let newLi = document.createElement("li")
		newLi.setAttribute('class', 'wrapperLi ')
		let img = document.createElement("img")
		img.setAttribute('src', arrFilm[i].poster)
		img.setAttribute('class', 'wrapperImg')
		newLi.appendChild(img)
		elemenFilm.appendChild(newLi)
		let elModal = document.querySelector(".modal")
		newLi.addEventListener("click", function (evt) {
			evt.preventDefault()
			mainListFirst.style.display = 'none';
		    elModal.classList.add("modal--active")
			let secondli = document.querySelector('#secondli')
			mainTitle.textContent = arrFilm[i].title
			mainText.textContent = arrFilm[i].overview
			let date = new Date(arrFilm[i].release_date)
			let secondGenres = arrFilm[i].genres
			let mainDay = String(date.getDay()).padStart(2, '0')
			let mainMonth = String(date.getMonth()).padStart(2, '0')
			let mainYear = date.getFullYear()
			mainTime.textContent = `${mainDay}.${mainMonth}.${mainYear}`
			mainGenres.textContent = secondGenres

			document.querySelector("#secondImg").setAttribute("src", arrFilm[i].poster)
            
			let scroll = document.querySelector("#secondli");
			scroll.scrollIntoView({behavior: 'smooth',block: 'end'
			});

		})
		let elSmallModal = document.querySelector(".modal-button")
		elSmallModal.addEventListener("click" , (event) =>{
...
elForm.addEventListener("submit", function (event) {
	...
	for (let i = 0; i < films.length; i++) {
		if (films[i].title.match(regex)) {
			mainListFirst.style.display = 'block'
			let mainList = document.querySelector('main-list')
			mainListTitle.textContent = films[i].title
			mainListText.textContent = films[i].overview
			let date = new Date(films[i].release_date)
			let secondGenres = films[i].genres
			let mainDay = String(date.getDay()).padStart(2, '0')
			let mainMonth = String(date.getMonth()).padStart(2, '0')
			let mainYear = date.getFullYear()
			mainListTime.textContent = `${mainDay}.${mainMonth}.${mainYear}`
			mainListGenres.textContent = secondGenres
			document.querySelector("#mainSecondImg").setAttribute("src", films[i].poster)
		}
	}
	elInput.value = ''
})

let elSelect = document.querySelector(".movie__genre")
...
function renderGenresSelect(films, element) {
	...
	result.map((genre) => {
		...
		newOption.setAttribute("class", "movie__option")
...
elSelect.addEventListener("change", function (event) {
	event.preventDefault()
	let selectGenres = elSelect.value.trim()
	let foundFilms = []
	if (selectGenres == "ALL") {
		foundFilms = films
	} else {
...
selectSort.addEventListener("change", (e) => {
	...
	function filterFilm(filmSort, format) {
		if (format === 'all') {
			return films
		}
		let filterBack = filmSort.sort((a, b) => {
			if (a.title > b.title) {
				return 1
		...
		if (format === 'a_z') {
			...
			return filterBack.reverse()
		}

		let filterDate = filmSort.sort((a, b) => {
	...
	let filteredfilms = filterFilm(films, selectSort.value.trim())
	render(filteredfilms, wrapperList)
